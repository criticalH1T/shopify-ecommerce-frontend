...
export class ShoppingCartComponent implements OnInit {
  ...
  shoppingCartList() {
    const finalResultArray = Object.values(this.shoppingCartService.getCartProducts().reduce((accumulator, product) => {
      ...
      accumulator[categoryKey][idKey].orderQuantitySubtotal += product.price;

      return accumulator;
    }, {}));

    // Flatten the nested structure into a single array
    this.shoppingCartItems = finalResultArray.flatMap(category => Object.values(category));
  }

  calculateSubtotal() {
    this.subtotalPrice = parseFloat(this.shoppingCartItems.reduce((sum, product) => sum + product.orderQuantitySubtotal, 0).toFixed(2));
  }

  removeItem(productId: number) {
    const index = this.shoppingCartItems.findIndex(product => product.id === productId);
